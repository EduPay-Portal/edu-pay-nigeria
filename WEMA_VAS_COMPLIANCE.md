...
| `WEMA_VAS_BEARER_TOKEN` | **Required from you** | Shared secret; the same value must be given to Wema |
| `WEMA_FALLBACK_BVN` / `WEMA_FALLBACK_NIN` | Optional | Institution's responsible-party identity for students without one |
...
1. `WEMA_VAS_BEARER_TOKEN` must be created and shared with Wema.
...
6. Existing legacy virtual accounts beginning with `9` predate this work and must be reissued before go-live.

**Not production ready** until items 1–5 are resolved with Wema.
